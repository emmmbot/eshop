...
public class OrderDao {
	...
	public void addProduct(Order order) throws SQLException {
		// 1.生成Sql语句
		String sql = "insert into orders(id,money,receiverAddress,receiverName,receiverPhone,paystate,ordertime,user_id) values(?,?,?,?,?,0,default,?)";
		...
		return runner.query(sql, new ResultSetHandler<List<Order>>() {
			public List<Order> handle(ResultSet rs) throws SQLException {
				...
				while (rs.next()) {
					...
					order.setTrackingnumber(rs.getString("orders.trackingnumber"));
					order.setUser(user);
					orders.add(order);
				}
				return orders;
	 ...
	 * @throws SQLException
	 */
	public void confirmDeliver(String id,String trackingnum) throws SQLException {
		String sql = "update orders set paystate=2,trackingnumber=? where id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		runner.update(sql, trackingnum, id);
		System.out.println(runner.update(sql, trackingnum, id));
	}
	/**
	 *  多条件查询
	 * @param id
