							...
							<td style="padding:20px"><p><b>我的订单</b></p>
								...
								<table width="100%" border="0" cellspacing="0" class="tableopen">
									<tr>
										...
										<td bgcolor="#A3B6E6" class="tableopentd01">运单号</td>
									...
									<c:forEach items="${orders}" var="order">
										<tr>
											...
											<td class="tableopentd02">${order.paystate==0?"未支付":''}${order.paystate==1?"已支付":""}${order.paystate==2?"已发货":""}</td>
											<td class="tableopentd02">${order.trackingnumber}</td>
											<td class="tableopentd03">
												<a href="${pageContext.request.contextPath}/findOrderById?id=${order.id}">查看</a>&nbsp;&nbsp;
												<c:if test="${order.paystate==0 }">
													<a href="${pageContext.request.contextPath}/delOrderById?id=${order.id}"  onclick="javascript:return o_del()">取消</a>
												</c:if>
												<!--
												<c:if test="${order.paystate!=0 }">
